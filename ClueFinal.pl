...
*/
check_accusation :-
	get_remaining_chars(C),
	get_remaining_weapons(W),
	get_remaining_rooms(R),
	length(C,1),
	length(W,1),
	length(R,1),
	write('Accusation found: \n'),
	write(C),
	write(' killed Mr. Boddy with '),
	write(W),
	write(' in the '),
	write(R),
	write('.').
	
get_remaining_chars(Chars) :-
	get_all_chars(C),
	get_eliminated_chars(EC),
	subtract(C,EC, Chars).
get_remaining_weapons(Weapons) :-
	get_all_weapons(W),
	get_eliminated_weapons(EW),
	subtract(W,EW, Weapons).
get_remaining_rooms(Rooms) :-
	get_all_rooms(R),
	get_eliminated_rooms(ER),
	subtract(R,ER,Rooms).
...
get_eliminated_chars(Chars) :-
	setof(X, (card_in_possession(_,X),character(X)), Chars).
get_eliminated_weapons(Weapons) :-
	setof(X, (card_in_possession(_,X),weapon(X)), Weapons).
get_eliminated_rooms(Rooms) :-
	setof(X, (card_in_possession(_,X),room(X)), Rooms).
...
save_own_cards([]).
save_own_cards([Card|Cards]) :-
	controlling_player(CPlayer),
	assert(card_in_possession(CPlayer, Card)),
	save_own_cards(Cards).
/*
    User Interface
*/
